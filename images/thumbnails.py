...
def get_sketch_thumbs(img, page):
  ...
  sketch_options = Filter.objects.filter(related_filter='sketch')[page_index_start:page_index_start + 10]
  
  for color in sketch_options:
    file_name = sketch(img, color.filter_option, thumbnail=True)
    encoded_image = str(base64.b64encode(open(f'{file_name}.png', 'rb').read()))[2:-1]
    thumb_dictionaries.append({'option': color.filter_option, 'image': f'data:image/png;base64,{encoded_image}'})
...
def get_histogram_thumbs(img, page):
  ...
  histogram_options = Filter.objects.filter(related_filter='histogram')[page_index_start:page_index_start + 10]
...
def get_collage_thumbs(img, page):
    ...
    for option in collage_options:
      ...
      encoded_image = str(base64.b64encode(open(f'{file_name}.png', 'rb').read()))[2:-1]
      thumb_dictionaries.append({'option': option.filter_option, 'image': f'data:image/png;base64,{encoded_image}'})
      os.remove(f'{file_name}.png')
    return thumb_dictionaries